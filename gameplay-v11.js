...
(function(){
const G={version:11,heat:0,calm:0,lastKill:0,frontierT:0,lastBiome:-1,salvage:0,choiceShown:false};window.GAMEPLAY_V11=G;
...
killMob=function(q){
  const V=window.GAMEPLAY_V10,c=V&&V.contract,def=mobDefs[q.type],special=c&&c.name!=='Cull'&&c.name!=='Frontier';
  if(special)V.contract=null; /* prevent v10's generic every-kill progress */
  preKill(q);
...
/* ---------- ADAPTIVE PRESSURE: HARD WITHOUT BEING CHEAP ---------- */
...
update=function(dt){
  ...
  panel.style.display='none';G.choiceShown=false;oldUpdate(dt);tickContract(dt);biomePulse();tutorialFix();
