...
class El{constructor(id=''){this.id=id;this.textContent='';this._html='';this.className='';this.classList=new CL();this.children=[];this.style={};this.listeners={};this.width=960;this.height=540}set innerHTML(v){this._html=v;if(v==='')this.children=[]}get innerHTML(){return this._html}addEventListener(t,fn){this.listeners[t]=fn}appendChild(c){this.children.push(c)}click(){const f=this.listeners.click||this.onclick;if(f)f({clientX:0,clientY:0})}dispatch(t,e){const f=this.listeners[t];if(f)f(e||{})}getBoundingClientRect(){return{left:0,top:0,width:960,height:540}}}
...
const ctx={imageSmoothingEnabled:false,fillStyle:'',strokeStyle:'',shadowColor:'',shadowBlur:0,lineWidth:1,font:'',textAlign:'',globalAlpha:1,createLinearGradient(){return{addColorStop(){}}},fillRect(){},beginPath(){},arc(){},fill(){},stroke(){},save(){},restore(){},translate(){},rotate(){},moveTo(){},lineTo(){},closePath(){},strokeRect(){},fillText(){}};
...
let seed=123456789;const gameMath=Object.create(Math);gameMath.random=()=>{seed=(seed*1664525+1013904223)>>>0;return seed/4294967296};
...
vm.createContext(sandbox);vm.runInContext(fs.readFileSync(__dirname+'/game-v5.js','utf8'),sandbox);
...
for(let frame=0;frame<15000;frame++){
  ...
  if(frame%60===0){el.game.dispatch('pointerdown',{clientX:165,clientY:270});taps++;}
  if(frame%180===0){el.game.dispatch('pointerdown',{clientX:760,clientY:300});taps++;}
}
const [m,s]=el.time.textContent.split(':').map(Number),seconds=m*60+s;
if(seconds<90)throw Error(`Run ended too early at ${el.time.textContent}`);
if(laterChoices>5)throw Error(`Too many paused choices: ${laterChoices}`);
console.log(`Starward V5 smoke OK: ${el.time.textContent}, later choices ${laterChoices}, taps ${taps}, hull ${el.hull.textContent}`);
